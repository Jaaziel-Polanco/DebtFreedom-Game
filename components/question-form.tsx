...
export function QuestionForm({ questionId }: QuestionFormProps) {
  ...
  function updateAnswer(index: number, field: keyof AnswerForm, value: string | boolean) {
      ...
      if (questionId) {
        ...
        for (const answer of answers) {
          if (answer.id) {
            ...
            const { error } = await supabase
              .from("answers")
              .insert({ question_id: questionId, text: answer.text, is_correct: answer.is_correct })
        ...
        const { data: maxOrderData } = await supabase.from("questions").select("display_order").order("display_order", { ascending: false }).limit(1)
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div className="flex gap-3">
        ...
        <Button type="button" variant="outline" size="lg" onClick={() => router.push("/admin/questions")}>
          Cancelar
        </Button>
      </div>
    </form>
