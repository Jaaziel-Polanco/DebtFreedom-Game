...
export default function LocalGamePlayPage() {
  ...
  const [animationType, setAnimationType] = useState<"win" | "lose" | null>(
    null
  );
  const [currentPrizeAmount, setCurrentPrizeAmount] = useState(0);
  const [preSelectedAnswer, setPreSelectedAnswer] = useState<string | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    Object.values(gameState.answeredQuestions).forEach(
      ({ isCorrect, prize }) => {
        if (!isCorrect) {
          hasIncorrect = true;
      ...
      {showAnimation && (
        <div
          className={`fixed inset-0 z-50 pointer-events-none flex items-center justify-center ${
            animationType === "win" ? "bg-green-500/10" : "bg-red-500/10"
          }`}
        >
          {animationType === "win" ? (
            <div className="text-center">
              ...
              <div className="text-3xl text-green-500 mt-2 animate-pulse">
                ¡CORRECTO!
              </div>
              {/* Confetti and emojis */}
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-lg font-bold text-yellow-600">
                      $
                      {prizes
                        .find((p) => p.question_id === currentQuestion.id)
                        ?.prize_amount.toLocaleString() || "0"}
                    </span>
                  </div>
                </div>
                <Progress value={progress} className="h-1.5" />
                <CardTitle className="text-2xl mt-3 leading-tight">
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <span className="font-bold text-sm">
                            ${prize?.prize_amount.toLocaleString() || "0"}
                          </span>
                        </div>
                      </div>
                    );
