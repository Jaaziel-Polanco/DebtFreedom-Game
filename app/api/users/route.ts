...
export async function POST(request: Request) {
  try {
    ...
    if (existingUser) {
      ...
      const { data, error } = await supabase.from("users").update({ name }).eq("phone", phone).select().single()

      if (error) throw error
      return NextResponse.json(data)
    } else {
      // Create new user
      const { data, error } = await supabase.from("users").insert({ name, phone }).select().single()
